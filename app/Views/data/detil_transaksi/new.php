...
<div class="card shadow mb-4">
	...
	<form action="/detil/create" method="post">
		<div class="card-body">
			<?= csrf_field(); ?>
			<div class="row">
				<div class="form-group col-md-6">
					...
					<select class="form-control select2bs4 <?= $validation->hasError('kode_transaksi') ? 'is-invalid' : '' ?>" style="width: 100%;" name="kode_transaksi">
						...
						<?php foreach($transaksi as $transaksi) : ?>
							<option value="<?= $transaksi['kode_transaksi'] ?>"><?= $transaksi['kode_transaksi'] ?> - <?= $transaksi['nama_pembeli'] ?></option>
						<?php endforeach; ?>
					</select>
				...
				<div class="form-group col-md-6">
					...
					<select class="form-control select2bs4 pilih-obat <?= $validation->hasError('kode_obat') ? 'is-invalid' : '' ?>" style="width: 100%;" name="kode_obat">
						...
						<?php foreach($obat as $obat) : ?>
							<option value="<?= $obat['kode_obat'] ?>" data-harga="<?= $obat['harga'] ?>"><?= $obat['nama_obat'] ?> - Rp. <?= $obat['harga'] ?></option>
						<?php endforeach; ?>
					</select>
					<div class="invalid-feedback">
